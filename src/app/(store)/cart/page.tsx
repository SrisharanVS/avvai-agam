...
import { Trash2, Minus, Plus, ShoppingBag, Leaf, ArrowRight, Scale } from "lucide-react";
...
export default function CartPage() {
  const { items, removeItem, updateQuantity, subtotal, totalShippingWeight } = useCartStore();
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <Link href={`/product/${item.slug}`}>
                    <h3 className="font-semibold text-gray-800 hover:text-primary-700 transition-colors line-clamp-1">
                      {item.productName}
                    </h3>
                  </Link>
                  {/* Variant name */}
                  <p className="text-sm text-primary-600 font-medium mt-0.5">{item.variantName}</p>
                  {/* Unit price + weight */}
                  <div className="flex items-center gap-3 mt-0.5">
                    <p className="text-xs text-muted-foreground">₹{item.price.toFixed(2)} each</p>
                    <p className="text-xs text-muted-foreground flex items-center gap-0.5">
                      <Scale className="w-3 h-3" /> {item.shippingWeight} kg/unit
                    </p>
                  </div>
              ...
              <div className="space-y-2.5 mb-4 text-sm text-gray-600">
                ...
                {shipping > 0 && (
                  <p className="text-xs text-muted-foreground bg-amber-50 px-3 py-2 rounded-lg">
                    💡 Add ₹{(freeShippingThreshold - sub).toFixed(0)} more for free shipping (TN)!
                  </p>
                )}
              </div>
